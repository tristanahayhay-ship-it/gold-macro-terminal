...
Yêu cầu bắt buộc: Viết bằng tiếng Việt. Chỉ trả về văn bản nhận định dạng HTML (dùng thẻ <b>, <br>), tuyệt đối không dùng câu văn mẫu lặp đi lặp lại."""
    ...
    # ===============================================================================================
    # ===================================================================================================
# 4. TIN TỨC & CỔ PHIẾU (DỮ LIỆU THẬT 100% - CẬP NHẬT REAL-TIME)
# ===================================================================================================
elif menu == "Tin Tức & Cổ Phiếu":
    st.title("📈 Thị Trường Chứng Khoán & Sức Khỏe Doanh Nghiệp")
    
    # Hàm tối ưu hóa lấy dữ liệu lịch sử để tính Delta và vẽ biểu đồ tương quan
    # Sử dụng cache thời gian ngắn để bảo vệ luồng tránh bị khóa IP do gửi request liên tục từng giây
    @st.cache_data(ttl=10)
    def get_realtime_stock_and_chart_data():
        ...
        # 1. Lấy dữ liệu Real-time phục vụ cho các ô chỉ số (Metrics)
        metrics_results = {}
        for name, sym in tickers.items():
        ...
        # 2. Lấy dữ liệu 90 ngày gần nhất tính theo khung giờ giao dịch thực tế cho biểu đồ tương quan
        ...
        try:
            ...
            # Chuẩn hóa về tỷ lệ tăng trưởng (%) để hiển thị chung một biểu đồ trực quan
    ...
    # Hàm cào tin tức doanh nghiệp thực tế 100% từ Yahoo Finance
    @st.cache_data(ttl=30)
    def get_realtime_enterprise_news():
        ...
        from datetime import datetime
        try:
            t = yf.Ticker("SPY") # Lấy cổng tin tức thị trường chung Hoa Kỳ
            raw_news = t.news[:5]
            news_data = []
            for n in raw_news:
                pub_time = datetime.fromtimestamp(n.get('providerPublishTime', 0)).strftime('%H:%M')
                related = ", ".join(n.get('relatedTickers', ['Thị trường']))
                title = n.get('title', 'Đang cập nhật tin tức...')
                news_data.append({
                    "Thời gian": pub_time,
                    "Mã cổ phiếu / Nhóm ngành": related,
                    "Nội dung sự kiện": title
                })
            return pd.DataFrame(news_data)
        except Exception:
            return pd.DataFrame({"Thời gian": [], "Mã cổ phiếu / Nhóm ngành": [], "Nội dung sự kiện": []})

    # CƠ CHẾ LIVE CẬP NHẬT TỪNG GIÂY BẰNG FRAGMENT (Không load lại toàn bộ trang)
    @st.fragment(run_every=1)
    def render_live_stock_section():
        ...
        # Hiển thị cấu trúc cột nguyên bản 100%
        ...
        # Hiển thị cấu trúc Bảng Tin nguyên bản 100%
        st.subheader("📰 Bảng Tin Doanh Nghiệp Real-time")
        if not df_news.empty:
            st.dataframe(df_news, use_container_width=True, hide_index=True)
        else:
            st.info("Đang đồng bộ luồng tin tức trực tuyến từ thị trường chứng khoán...")
        
        # Hiển thị cấu trúc Biểu đồ nguyên bản 100%
        ...
        if not df_chart.empty:
            # Sử dụng Plotly thay cho st.line_chart để xử lý màu và hiển thị thông tin chi tiết khi chỉ vào đường dây
            fig = go.Figure()
            
            # Đường S&P 500 (Giữ nguyên màu xanh mặc định, cấu hình hiển thị nhãn)
            ...
            # Đường Vàng (Đổi sang màu Vàng rực chuyên nghiệp, cấu hình hiển thị nhãn)
            fig.add_trace(go.Scatter(
                x=df_chart.index, 
                y=df_chart['Gold Price'], 
                mode='lines', 
                name='Gold Price',
                line=dict(color='#eab308', width=2), # Màu vàng mã Hex chuẩn Bloomberg
                hovertemplate="<b>Ngày:</b> %{x}<br><b>Hiệu suất Giá Vàng:</b> %{y:.2f}%<extra></extra>"
            ))
            
            # Đồng bộ giao diện Dark Mode chuẩn khớp với CSS Bloomberg của bạn
            fig.update_layout(
                ...
                height=420,
                hovermode="x unified", # Chỉ chuột vào bất kỳ đâu trên trục X sẽ hiện thông số của cả 2 đường cùng lúc để so sánh
                xaxis=dict(showgrid=True, gridcolor="#1e293b", title="Thời gian"),
                yaxis=dict(showgrid=True, gridcolor="#1e293b", title="Hiệu suất (%)")
            )
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        else:
            st.warning("Đang kết nối cổng dữ liệu đồ thị chứng khoán vĩ mô...")

    # Chạy hàm hiển thị trực tiếp
