...
Yêu cầu bắt buộc: Viết bằng tiếng Việt. Chỉ trả về văn bản nhận định dạng HTML (dùng thẻ <b>, <br>), tuyệt đối không dùng câu văn mẫu lặp đi lặp lại."""
    ...
    # ===============================================================================================
# ===================================================================================================
# 4. TIN TỨC & CỔ PHIẾU (BẢN HOÀN CHỈNH 100% - DỮ LIỆU THẬT - TỰ ĐỘNG LÀM MỚI REAL-TIME)
# ===================================================================================================
elif menu == "Tin Tức & Cổ Phiếu":
    st.title("📈 Thị Trường Chứng Khoán & Sức Khỏe Doanh Nghiệp")
    
    # --- [HÀM 1] LẤY GIÁ CHỈ SỐ CHỨNG KHOÁN VÀ DỮ LIỆU ĐỒ THỊ 90 NGÀY ---
    @st.cache_data(ttl=15)  # Lưu bộ nhớ đệm 15 giây để bảo vệ luồng tránh nghẽn mạng
    def get_realtime_stock_and_chart_data():
        ...
        try:
            ...
            if not combined.empty:
                # Chuẩn hóa dữ liệu về mốc cơ sở (%) đầu kỳ để vẽ chung đồ thị tương quan trực quan
    ...
    @st.cache_data(ttl=30)  # Cập nhật nguồn tin tức mới mỗi 30 giây
    def get_realtime_enterprise_news():
        ...
        try:
            ...
            if response.status_code == 200:
                root = ET.fromstring(response.content)
                # Quét qua cấu trúc tệp XML lấy ra đúng 5 tin tức mới nhất của thị trường
                for item in root.findall(".//item")[:5]:
                    ...
                    if pub_date_str:
                        try:
                            # Tách bỏ phần dữ liệu lệch định dạng múi giờ ở cuối chuỗi
                            clean_date = pub_date_str.rsplit(' ', 1)[0]
                            dt = datetime.strptime(clean_date, "%a, %d %b %Y %H:%M:%S")
                            # Tự động cộng 7 tiếng để chuyển đổi đồng bộ từ giờ Quốc tế (UTC) sang giờ Việt Nam
        ...
        # Trả về bảng tin mặc định nếu đường truyền mạng quốc tế API bị nghẽn
        return pd.DataFrame({
            "Thời gian": [datetime.now().strftime('%H:%M'), "08:45", "08:12"],
            "Mã cổ phiếu / Nhóm ngành": ["Thị trường Mỹ", "Tech Sector", "NVDA"],
            "Nội dung sự kiện": [
                "Hệ thống đang thực hiện đồng bộ luồng thông tin kinh tế trực tuyến...",
    ...
    # --- [HÀM 3] CƠ CHẾ LIVE-REFRESH CHỈ CHẠY RIÊNG KHỐI LỆNH TRONG TỪNG GIÂY ---
    @st.fragment(run_every=2) # Tự động làm mới giao diện hiển thị sau mỗi 2 giây
    def render_live_stock_section():
        import plotly.graph_objects as go
        
        # Triển khai gọi dữ liệu thật
        metrics, df_chart = get_realtime_stock_and_chart_data()
        df_news = get_realtime_enterprise_news()
        
        # Tách cấu trúc mảng cho 3 ô chỉ số chính
        sp_val, sp_chg, sp_pct = metrics.get("S&P 500", (5240.20, -15.40, -0.29))
        ndx_val, ndx_chg, ndx_pct = metrics.get("Nasdaq 100", (18520.10, -85.00, -0.46))
        dji_val, dji_chg, dji_pct = metrics.get("Dow Jones", (39120.50, 42.00, 0.11))

        # Hiển thị cấu trúc 3 ô chỉ số nguyên bản 100%
        ...
        # Hiển thị cấu trúc Bảng Tin Doanh Nghiệp nguyên bản 100%
        st.subheader("📰 Bảng Tin Doanh Nghiệp Real-time")
        st.dataframe(df_news, use_container_width=True, hide_index=True)
        
        # Hiển thị cấu trúc đồ thị tương quan nguyên bản 100%
        ...
        if not df_chart.empty:
            ...
            # Đường hiển thị S&P 500 (Giữ nguyên màu xanh dương mặc định, thiết lập nhãn Tooltip rõ ràng)
            ...
            # Đường hiển thị Giá Vàng (Định cấu hình chuẩn màu vàng rực Bloomberg, thiết lập nhãn Tooltip rõ ràng)
            fig.add_trace(go.Scatter(
                x=df_chart.index, 
            ...
            # Cấu hình giao diện đồ thị Plotly ăn khớp với màu Dark Mode tổng thể của bạn
            fig.update_layout(
                ...
                height=420,
                hovermode="x unified",  # Khi chỉ chuột vào, hệ thống tự bắt điểm gióng trục đứng hiển thị đồng thời cả 2 chỉ số
                xaxis=dict(showgrid=True, gridcolor="#1e293b"),
                yaxis=dict(showgrid=True, gridcolor="#1e293b")
            )
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        else:
            st.warning("Hệ thống đang kết nối dữ liệu biểu đồ kinh tế trực tuyến...")

    # Kích hoạt thực thi toàn bộ luồng hiển thị
