...
# 1. CẤU HÌNH GIAO DIỆN HỆ THỐNG TERMINAL TOÀN CẦU
st.set_page_config(
...
# 2. BỘ ĐIỀU KHIỂN SIDEBAR TƯƠNG TÁC ĐA LỚP
...
if selected_country == "Việt Nam":
    ...
    assets_data = {
        ...
        "Tiền mặt & Tiết kiệm": {"name": "Hệ thống Ngân hàng Thương mại (Tiết kiệm VND)", "lat": 21.0167, "lon": 105.8167, "type": "SAFE_HAVEN"},
    ...
    geo_config = dict(projection_type="mercator", center=dict(lat=37.0, lon=-95.7), zoom=3.0)
    ...
    geo_config = dict(projection_type="natural earth", center=dict(lat=20, lon=0), zoom=1.2)
...
fig.add_trace(go.Scattergeo(
    ...
    text = f"🏛️ CẤP 1 (THƯỢNG TẦNG): {level_1['name']}",
...
for agent_key in active_agents:
    ...
    fig.add_trace(go.Scattergeo(
        ...
        text = f"👤 CẤP 2 (CHỦ THỂ): {agent['name']}",
...
for asset_key, asset in assets_data.items():
    ...
    fig.add_trace(go.Scattergeo(
        ...
        text = f"🎯 CẤP 3 (TÀI SẢN): {asset['name']} [{asset['type']}]",
...
for agent_key in active_agents:
    ...
    for asset_key, asset in assets_data.items():
        if asset["type"] == flow_type:
            ...
            path_lat = [agent["lat"], asset["lat"]] if flow_type == "INVESTMENT" else [asset["lat"], agent["lat"]]
