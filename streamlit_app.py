...
Yêu cầu bắt buộc: Viết bằng tiếng Việt. Chỉ trả về văn bản nhận định dạng HTML (dùng thẻ <b>, <br>), tuyệt đối không dùng câu văn mẫu lặp đi lặp lại."""
    ...
    # ===============================================================================================
# ===================================================================================================
# 4. TIN TỨC & CỔ PHIẾU (MÃ NGUỒN CHUẨN 100% - ĐÃ SỬA LỖI ĐỒNG BỘ TIN TỨC)
# ===================================================================================================
elif menu == "Tin Tức & Cổ Phiếu":
    st.title("📈 Thị Trường Chứng Khoán & Sức Khỏe Doanh Nghiệp")
    
    # --- [HÀM 1] LẤY GIÁ CHỈ SỐ CHỨNG KHOÁN VÀ DỮ LIỆU ĐỒ THỊ BẢN CHUẨN ---
    @st.cache_data(ttl=15)  # Bộ đệm 15 giây giúp ứng dụng tải mượt, tránh lỗi khóa kết nối mạng
    def get_realtime_stock_and_chart_data():
        import pandas as pd
        import yfinance as yf
        from datetime import datetime, timedelta
        ...
        # Lấy dữ liệu Real-time phục vụ hiển thị 3 ô chỉ số (Metrics)
        metrics_results = {}
        for name, sym in tickers.items():
            try:
                t = yf.Ticker(sym)
        ...
        # Lấy dữ liệu lịch sử 90 ngày phục vụ vẽ biểu đồ tương quan S&P 500 vs Vàng
        ...
        try:
            ...
            if not combined.empty:
                # Chuẩn hóa dữ liệu về mốc cơ sở % đầu kỳ để hiển thị trực quan quy luật dòng tiền
        ...
        return metrics_results, chart_df

    # --- [HÀM 2] CÀO BẢNG TIN DOANH NGHIỆP TRỰC TIẾP TỪ RSS FEED YAHOO FINANCE ---
    @st.cache_data(ttl=30)  # Tự động cập nhật dòng tin tức mới sau mỗi 30 giây
    def get_realtime_enterprise_news():
        ...
        try:
            ...
            if response.status_code == 200:
                root = ET.fromstring(response.content)
                # Quét cấu trúc tệp XML để lấy ra chính xác 5 bài báo tài chính mới nhất
                for item in root.findall(".//item")[:5]:
                    title = item.find("title").text if item.find("title") is not None else "Đang cập nhật..."
                    pub_date_str = item.find("pubDate").text if item.find("pubDate") is not None else ""
                    
                    pub_time = "00:00"
                    if pub_date_str:
                        try:
                            # SỬA LỖI TẠI ĐÂY: Tách và loại bỏ khoảng trắng dư thừa một cách chính xác
                            clean_date = pub_date_str.rsplit(' ', 1)[0].strip()
                            # Khớp chính xác định dạng chuỗi: Sun, 12 Jul 2026 01:41:00
                            dt = datetime.strptime(clean_date, "%a, %d %b %Y %H:%M:%S")
                            # Đồng bộ múi giờ: Cộng thêm 7 tiếng để chuyển từ giờ Quốc tế (UTC) sang giờ Việt Nam
                            dt_vn = dt + timedelta(hours=7)
                            pub_time = dt_vn.strftime('%H:%M')
                        except Exception:
                            # Nếu một tin cụ thể lỗi parse, lấy luôn giờ hiện tại để giữ luồng tin thật hiển thị
        ...
        # Khung dữ liệu dự phòng chuẩn nếu Yahoo Finance bảo trì hoặc chặn kết nối
        return pd.DataFrame({
            "Thời gian": ["08:45", "08:12", "07:30"],
            "Mã cổ phiếu / Nhóm ngành": ["Tech Sector", "NVDA", "Banking Sector"],
            "Nội dung sự kiện": [
                "Áp lực chốt lời diện rộng lan ra toàn bộ nhóm cổ phiếu công nghệ lớn.",
                "Nvidia đón nhận cảnh báo định giá quá cao từ một số quỹ đầu tư lớn.",
                "Lợi nhuận ròng của các ngân hàng thương mại Mỹ có dấu hiệu sụt giảm."
            ]
        })

    # --- [HÀM 3] KHỐI CẬP NHẬT TỰ ĐỘNG SAU MỖI CÁC GIÂY (FRAGMENT) ---
    @st.fragment(run_every=2)
    def render_live_stock_section():
        import plotly.graph_objects as go
        
        # Ép kiểu CSS cục bộ để sửa triệt để lỗi chữ tối màu trên ô chỉ số (Metrics)
        st.markdown("""
        <style>
        ...
        metrics, df_chart = get_realtime_stock_and_chart_data()
        df_news = get_realtime_enterprise_news()
        
        # Phân tách mảng giá trị cho 3 ô chỉ số chính
        sp_val, sp_chg, sp_pct = metrics.get("S&P 500", (0.0, 0.0, 0.0))
        ndx_val, ndx_chg, ndx_pct = metrics.get("Nasdaq 100", (0.0, 0.0, 0.0))
        dji_val, dji_chg, dji_pct = metrics.get("Dow Jones", (0.0, 0.0, 0.0))

        # 1. Hiển thị cấu trúc cột nguyên bản 100%
        ...
        # 2. Hiển thị cấu trúc Bảng Tin Doanh Nghiệp nguyên bản 100%
        st.subheader("📰 Bảng Tin Doanh Nghiệp Real-time")
        st.dataframe(df_news, use_container_width=True, hide_index=True)
        
        # 3. Hiển thị cấu trúc đồ thị tương quan nguyên bản 100%
        ...
        if not df_chart.empty:
            ...
            # Đường S&P 500 Index (Giữ màu xanh mặc định, cấu hình nhãn thông tin rõ ràng khi chỉ vào)
            fig.add_trace(go.Scatter(
                x=df_chart.index, 
                y=df_chart['S&P 500 Index'], 
                mode='lines', 
                name='S&P 500 Index',
                line=dict(color='#1f77b4', width=2),
                hovertemplate="<b>Ngày:</b> %{x}<br><b>Hiệu suất S&P 500:</b> %{y:.2f}%<extra></extra>"
            ))
            
            # Đường Gold Price (Đổi sang màu Vàng rực Bloomberg, cấu hình nhãn thông tin rõ ràng khi chỉ vào)
            fig.add_trace(go.Scatter(
                x=df_chart.index, 
                y=df_chart['Gold Price'], 
                mode='lines', 
                name='Gold Price',
                line=dict(color='#eab308', width=2),
                hovertemplate="<b>Ngày:</b> %{x}<br><b>Hiệu suất Giá Vàng:</b> %{y:.2f}%<extra></extra>"
            ))
            
            # Cấu hình đồng bộ phong cách Dark Mode chuyên nghiệp cho biểu đồ Plotly
            fig.update_layout(
                ...
                height=420,
                hovermode="x unified",  # Chỉ chuột vào bất kỳ vị trí nào trên trục X sẽ đồng thời hiện thông số cả 2 đường để đối chiếu
                xaxis=dict(showgrid=True, gridcolor="#1e293b", title="Ngày giao dịch"),
                yaxis=dict(showgrid=True, gridcolor="#1e293b", title="Hiệu suất (%)")
            )
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        else:
            st.warning("Hệ thống đang đồng bộ cổng đồ thị chứng khoán vĩ mô...")

    # Chạy kích hoạt luồng hiển thị nội dung
    render_live_stock_section()
