...
# ===================================================================================================
    ...
    # Khối hiển thị biểu đồ TradingView Live
    ...
    st.markdown("---")
    st.subheader("🤖 Bot Phân Tích Gợi Ý Tín Hiệu Tự Động")
    st.caption("Hệ thống đồng bộ dữ liệu Realtime - Không cần nhập thủ công")

    # Tạo nút bấm để người dùng có thể chủ động ép làm mới dữ liệu khi cần
    refresh_button = st.button("🔄 Làm mới dữ liệu thị trường tức thì")

    # TỰ ĐỘNG QUÉT DỮ LIỆU TỪ YAHOO FINANCE (Khung 1 giờ cho điểm đáy ngắn hạn)
    with st.spinner("🚀 Đang tự động kết nối API và đồng bộ chỉ số kỹ thuật Live..."):
        try:
            # Lấy dữ liệu mã hợp đồng Vàng tương lai (GC=F) tương đương XAU/USD toàn cầu
            gold_ticker = yf.Ticker("GC=F")
            df = gold_ticker.history(period="1mo", interval="1h")

            if df.empty:
                st.error("❌ Không thể kết nối tới máy chủ dữ liệu. Vui lòng kiểm tra internet hoặc thử lại.")
                st.stop()

            # Tính toán tự động các chỉ báo thông qua pandas_ta
            df['RSI'] = ta.rsi(df['Close'], length=14)
            macd_data = ta.macd(df['Close'])
            df['MACD'] = macd_data['MACD_12_26_9']
            df['MA20'] = ta.sma(df['Close'], length=20)
            df['MA20_Vol'] = ta.sma(df['Volume'], length=20)

            # Trích xuất dữ liệu của nến đóng cửa gần nhất (Live) và nến trước đó
            latest_bar = df.iloc[-1]
            prev_bar = df.iloc[-2]

            # Gán giá trị tự động hoàn toàn cho thuật toán
            current_gold_price = round(float(latest_bar['Close']), 2)
            user_rsi = round(float(latest_bar['RSI']), 1)
            user_macd = round(float(latest_bar['MACD']), 2)
            user_ma20 = round(float(latest_bar['MA20']), 2)
            user_volume = int(latest_bar['Volume'])
            user_avg_vol = int(latest_bar['MA20_Vol'])
            
            # Tự động xác định màu sắc dòng tiền dựa trên nến tăng/giảm
            if latest_bar['Close'] >= prev_bar['Close']:
                volume_color = "🟢 XANH (Lực mua chiếm ưu thế)"
            else:
                volume_color = "🔴 ĐỎ (Lực bán chiếm ưu thế)"

        except Exception as e:
            st.error(f"⚠️ Đã xảy ra lỗi khi đồng bộ dữ liệu tự động: {e}")
            st.stop()

    # HIỂN THỊ THÀNH PHẦN THÔNG SỐ ĐÃ ĐƯỢC ĐỒNG BỘ (Chuyển sang dạng read-only để tăng trải nghiệm)
    ...
    # THUẬT TOÁN TÍNH TOÁN ĐIỂM SỐ TRỌNG SỐ TỰ ĐỘNG
    total_score = 5.0
    analysis_logs = []
    ...
    # Hiển thị hộp tín hiệu đổi màu thông minh dựa trên hàm thông báo chuẩn của Streamlit
    if color_status == "success":
        st.success(f"🎯 **TÍN HIỆU THUẬT TOÁN ĐỘNG: {signal}**")
    elif color_status == "error":
    ...
    st.write(f"📊 **Dòng tiền Volume:** Phiên hiện tại đạt **{user_volume}** đơn vị (Đang bằng **{vol_ratio:.2f} lần** so với mức trung bình MA20 là {user_avg_vol}).")
