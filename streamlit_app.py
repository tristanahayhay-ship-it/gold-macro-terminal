...
# ===================================================================================================
    ...
    # =========================================================================
    # PHẦN 2: THUẬT TOÁN TỰ ĐỘNG ĐỌC CHỈ SỐ & PHÁT TÍN HIỆU THỰC CHIẾN (VIẾT TIẾP)
    # =========================================================================
    ...
    st_autorefresh(interval=1000, limit=None, key="pure_algo_scanner_stream")

    st.markdown("---")
    st.subheader("🤖 Bot Thuật Toán Phân Tích & Phát Tín Hiệu Tự Động")
    st.caption("Thuật toán tự động bóc tách chuỗi nến thực tế từ sàn giao dịch, tính toán công thức gốc và ra kết luận")

    # 2. HÀM THUẬT TOÁN TỰ ĐỘNG QUÉT VÀ XỬ LÝ TOÁN HỌC CHỈ BÁO KỸ THUẬT
    def execute_live_trading_algorithm():
        try:
            # Tải 50 cây nến 1 phút mới nhất của PAXG/USDT (Dữ liệu Vàng chạy 24/7 thực tế)
            url = "https://mexc.com"
            response = requests.get(url, timeout=1.5).json()
            
            # Khởi tạo bảng dữ liệu Pandas để tính toán
            df = pd.DataFrame(response, columns=['Time', 'Open', 'High', 'Low', 'Close', 'Volume', 'CloseTime', 'AssetVol', 'Trades', 'BuyBase', 'BuyQuote', 'Ignore'])
            ...
            # --- THUẬT TOÁN TÍNH TOÁN CÔNG THỨC CHỈ BÁO GỐC CHUẨN XÁC ---
            # Tính đường trung bình xu hướng MA20
            ...
            # --- THUẬT TOÁN BIỆN LUẬN LOGIC ĐỂ XUẤT TÍN HIỆU TỰ ĐỘNG ---
            if rsi_val < 30 and current_price > ma20:
                signal = "MUA (BUY)"
                color = "green"
                reason = f"Chỉ số RSI lọt vào vùng quá bán ({rsi_val}) cho thấy lực xả đã cạn. Đồng thời giá giữ vững trên trục xu hướng MA20, thuật toán xác nhận phe Mua đang chiếm lại quyền kiểm soát."
            elif rsi_val > 70 and current_price < ma20:
                signal = "BÁN (SELL)"
                color = "red"
                reason = f"Chỉ số RSI rơi vào vùng quá mua nguy hiểm ({rsi_val}). Đồng thời cấu trúc giá đánh thủng đường trung bình MA20, thuật toán xác nhận áp lực tháo chạy của dòng tiền lớn."
            else:
                signal = "ĐỨNG NGOÀI (WAIT)"
                color = "orange"
                reason = f"Đường giá và các chỉ báo đang tích lũy trong biên độ an toàn (RSI: {rsi_val} | MACD: {macd_val} | MA20: ${ma20}). Không có tín hiệu đột biến, khuyến nghị giữ kỷ luật đứng ngoài quan sát."
            ...
            return {"success": False, "price": 2354.50, "rsi": 50.0, "macd": 0.0, "ma20": 2350.0, "signal": "ĐANG KẾT NỐI...", "color": "orange", "reason": "Hệ thống đang đồng bộ tuyến cáp API"}

    # Khởi chạy bộ quét thuật toán
    algo_output = execute_live_trading_algorithm()

    # 3. HIỂN THỊ CÁC Ô CHỈ BÁO SỐ THỰC TẾ (Tự nhảy số tanh tách mỗi giây)
    st.write(f"💵 Giá thực tế đang khớp trên sổ lệnh giây này: **${algo_output['price']}**")
    
    ind_col1, ind_col2, ind_col3 = st.columns(3)
    with ind_col1:
        st.metric(label="📊 Chỉ số RSI (14) Thực tế", value=str(algo_output['rsi']))
    with ind_col2:
        st.metric(label="📉 Chỉ số MACD Thực tế", value=str(algo_output['macd']))
    with ind_col3:
        st.metric(label="📈 Đường Xu hướng MA(20)", value=f"${algo_output['ma20']}")
        
    # 4. HỘP KẾT LUẬN TÍN HIỆU TỰ ĐỘNG ĐỔI MÀU THEO PHÁN QUYẾT CỦA THUẬT TOÁN
    if algo_output["success"]:
        if algo_output["color"] == "green":
            st.success(f"🎯 **TÍN HIỆU TỰ ĐỘNG: {algo_output['signal']}**")
        elif algo_output["color"] == "red":
            st.error(f"🎯 **TÍN HIỆU TỰ ĐỘNG: {algo_output['signal']}**")
        else:
            st.warning(f"🎯 **TÍN HIỆU TỰ ĐỘNG: {algo_output['signal']}**")
            
        st.info(f"📝 **Lý giải logic thuật toán:** {algo_output['reason']}")
    else:
        st.info("🔄 Đang nạp luồng nến dữ liệu sống từ máy chủ toàn cầu...")
