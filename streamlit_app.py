...
Yêu cầu bắt buộc: Viết bằng tiếng Việt. Chỉ trả về văn bản nhận định dạng HTML (dùng thẻ <b>, <br>), tuyệt đối không dùng câu văn mẫu lặp đi lặp lại."""
    ...
    # ===============================================================================================
# ===================================================================================================
# 4. TIN TỨC & CỔ PHIẾU (BẢN FIX TRIỆT ĐỂ LỖI KẾT NỐI BẢNG TIN BẰNG FEEDPARSER)
# ===================================================================================================
    ...
    # --- [HÀM 2] CÀO BẢNG TIN DOANH NGHIỆP TRỰC TIẾP BẰNG THƯ VIỆN FEEDPARSER CHUYÊN DỤNG ---
    @st.cache_data(ttl=30)
    def get_realtime_enterprise_news():
        ...
        import time
        try:
            # Sử dụng luồng tin quốc tế tổng hợp ổn định của Yahoo Finance
            url = "https://yahoo.com"
            feed = feedparser.parse(url)
            
            news_data = []
            # Lặp qua các bài tin xuất bản từ feed của Yahoo
            for entry in feed.entries[:5]: # Trích xuất chuẩn 5 tin mới nhất
                title = entry.get('title', 'Đang cập nhật...')
                
                # Ép xử lý cấu trúc thời gian chuẩn hóa tự động từ thư viện
                pub_time = "00:00"
                if 'published_parsed' in entry:
                    try:
                        gmt_time = datetime(*entry.published_parsed[:6])
                        # Chuyển đổi múi giờ chuẩn quốc tế sang múi giờ Việt Nam (GMT+7)
                        vn_time = gmt_time + timedelta(hours=7)
                        pub_time = vn_time.strftime('%H:%M')
                    except Exception:
    ...
    def render_live_stock_section():
        ...
        if not df_news.empty:
            ...
            st.error("Lỗi kết nối cổng RSS. Vui lòng kiểm tra lại cấu hình thư viện feedparser.")
